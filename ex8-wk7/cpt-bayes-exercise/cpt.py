...
def get_conditional_probability(data, query, condititions):
    ...
    molecular = get_data_frequency(data, combined_dictionary)
    denominator = get_data_frequency(data, condititions)
    return molecular / (denominator + epsilon)
    # =========================================================
...
def exhaustive_enum(G, X, E, e):
    ...
    for assignment in assignments:
        ...
        for node in G.nodes:
            ...
            # CODE HERE!
            # =========================================================
            condition = {k: combined_dictionary[k] for k in node.parents}

            p = evaluate_node(G, node.data_id, combined_dictionary[node.data_id], condition) # graph, int , int, dict
            tmp *= p
        total += tmp
    return total
