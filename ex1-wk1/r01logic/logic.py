...
def powerset(string):
    ...
    for i in range(0,n+1):
        for element in itertools.combinations(string,i):
            res_ls.append(list(element))
    return res_ls

def satisfiable(f):
  all_subset = powerset(f.vars()) # find the names of all atomic formulas
  for sets in all_subset:
    if f.truthValue(sets): return True
  return False
...
def logicalConsequence(f1,f2):
  # return not satisfiable(NOT(IMPL(f1,f2)))
  return not satisfiable(NOT(IMPL(f1,f2)))
### IMPLEMENT THIS (~ 1 line)
