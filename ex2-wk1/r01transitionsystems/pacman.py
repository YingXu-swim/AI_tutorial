...
class PacManState:
    ...
    def successors(self):
        ...
        directions = ["N","S", "W", "E"]
        for d in directions:
            move_state_change = self.move(d)
            move_state = (self.x+move_state_change[0],self.y+move_state_change[1],move_state_change[-1])
            if not self.grid.occupied(move_state[0], move_state[1]):
                res_ls.append((str(self.d)+"->"+str(d), PacManState(move_state[0], move_state[1], d, self.grid)))
        return res_ls
