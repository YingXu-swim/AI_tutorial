...
def unit_propagation(valuation, clauses):
    ...
    * Otherwise, if all literals of a clause are False, then the whole clause
    ...
    # To avoid recurssion error
    valuation_backup = valuation.copy()

    # explicitly skip true clauses
    # true_clause = []
    ...
    while loop:
        ...
        for clause in clauses:
            ...
            # if clause in true_clause:
            #     continue

            unassigned_literal = []

            count_false_literal = 0

            # only one literal is unassigned
            for literal, value in clause:
                if literal not in valuation.keys():
                    unassigned_literal.append(literal)
                    if len(unassigned_literal) > 1:
                        break
                    unassigned_value = value
                else:
                    # If there is a literal being true under the valuation, the clause is true and can be skipped.
                    # But explicitly skipping true clauses reduces computational efficiency.
                    if valuation[literal] == value:
                        # true_clause.append(clause)
                        break
                    count_false_literal += 1

            if (len(unassigned_literal) == 1) and (count_false_literal == len(clause) - 1):
                unit_clause.append(clause)
                # Use valuation_backup to avoid recurssion error
                valuation_backup[unassigned_literal[0]] = unassigned_value

        # Update the original valuation separately
        valuation = valuation_backup
        if unit_clause == []:
            loop = False
    ...
    for clause in clauses:
        ...
        count_false_literal = 0
        for literal, value in clause:
            if literal in valuation.keys():
                if valuation[literal] == (not value):
                    count_false_literal += 1
    
        if len(clause) == count_false_literal:
            return (False, {})
...
def dpll(valuation, clauses, variables):
    ...
    # Use valuation_new to keep the original valuation in case the recursive result is unsatisfied
    satisfiability, valuation_new = dpll(valuation, clauses, variables)
    
    if satisfiability:
        return (satisfiability, valuation_new)
    
    valuation[branching_variable] = False
    return dpll(valuation, clauses, variables)