        ...
        for clause in clauses:
            ...
            for literal, value in clause:
                if literal not in valuation:
                    if len(unassigned_literal) == 0:
                        unassigned_literal = literal
                    else:
                        unassigned_literal = literal
                elif valuation[literal] != value:
                    count_false_literal += 1
            if count_false_literal == 0 and len(unassigned_literal) != 0:
                unit_clause = (unassigned_literal, clause[unassigned_literal])
                break
        
        if len(unit_clause) == 0:
            break
        literal, value = unit_clause
        valuation[literal] = value
        
    for clause in clauses:
        count_false_literal = 0
        for literal, value in clause:
            if literal in valuation and valuation[literal] != value:
                count_false_literal += 1
        if count_false_literal == len(clause):
            return (False, {})
