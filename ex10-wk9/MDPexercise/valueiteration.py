...
def value_of(mdp, s, a, v, gamma):
    ...
    new_v = 0
    for s_prime in mdp.successor_states(s, a):
        p = mdp.P(s, a, s_prime)
        r = mdp.R(s, a, s_prime)
        # try:
...
def value_iteration(mdp, gamma, epsilon=0.001):
    ...
    while True:
      for s in mdp.states():
         max_q = max(value_of(mdp, s, a, v, gamma) for a in mdp.applicable_actions(s))
         v_next[s] = max_q

      # temp_max = -99999
      # for s in mdp.states():
      #    if abs(v_next[s] - v[s]) > temp_max:
      #       temp_max = abs(v_next[s] - v[s])

      threshold = epsilon * (1 - gamma) / (2 * gamma)
      if all(abs(v_next[s] - v[s]) < threshold for s in mdp.states()):
         v = v_next.copy()
         break
      else:
         v = v_next.copy()
