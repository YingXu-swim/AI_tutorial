    ...
    while not Q.empty():
        ...
        for aname,s,cost in state.successors(): # Go through all successors of state
            if s not in visited: # Was the state visited already?
                visited[s] = 1
                #### YOUR CODE HERE ####
                d_dic[s] = d_dic[state] + cost

                Q.put( s )
            # elif d_dic[s] > d_dic[state] + cost:
            #     d_dic[s] = d_dic[state] + cost
    return d_dic
...
def makePDBheuristic(goalState,tiles):
    ...
    abstracted = goalState.abstract(tiles)
    print("abstracted", abstracted)
    lb = breadthFirstSearch(abstracted)

    return lambda s: lb[s.abstract(tiles)]
...
def makePDBheuristic2(goalState,tiles1,tiles2):
    #### YOUR CODE HERE ####
    abstracted1 = goalState.abstract(tiles1)
    abstracted2 = goalState.abstract(tiles2)
    lb1 = breadthFirstSearch(abstracted1)
    lb2 = breadthFirstSearch(abstracted2)

    if tiles1.intersection(tiles2) == set(): # 
        return lambda s: lb1[s.abstract(tiles1)] + lb2[s.abstract(tiles2)]
    else:
        return lambda s: max(lb1[s.abstract(tiles1)], lb2[s.abstract(tiles2)])
