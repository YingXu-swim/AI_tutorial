...
def variable(task : str, time_slot : int) -> ATOM:
    """
...
def at_most_one(formulas):
    ...
    return AND([
        NOT(AND([x, y])) for x, y in all_pairs(formulas)
    ])
...
def exactly_one_execution(tasks, time_interval):
    ...
    return AND([
        exactly_one([   variable(task, t) for t in range(time_interval[0], time_interval[1])   ]) 
        for task in tasks
    ])
...
def mutually_exclusive_tasks(tasks, resource_need, time_interval):
    ...
    return AND([
        ...
        for task1, task2 in all_pairs(tasks) if resource_need[task1] == resource_need[task2]
    ])
