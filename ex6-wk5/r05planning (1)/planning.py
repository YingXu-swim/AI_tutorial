...
def findPlan(source,transition,target,MAX : int):
    ...
    for T in range(0,MAX+1):
        ...
        #### YOUR CODE HERE
        ## formula = ....
        source_formula = instantiate(source, 0)

        target_formula = instantiate(target, T)

        transition_formulas = [instantiate(transition, t) for t in range(0, T)]

        formula = AND([source_formula, target_formula] + transition_formulas)
